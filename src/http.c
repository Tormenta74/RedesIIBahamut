...
int http_response_body(char *buf, char **body) {
    ...
    pointer = buf;
    do {
        /* "pointer" points to some character in the buffer "buf", and we print in "sequence" the string generated by
        *pointer, *pointer+1, *pointer+2 and *pointer+3, a 4-character string starting at address "pointer" */
        sprintf(sequence, "%.*s", 4, pointer);
        /* then, we increase "pointer" to point to the next character of the buffer */
        pointer++;
    /* if we encounter "\r\n\r\n" or '\0' we have finished our duty */
    } while (strcmp(sequence, "\r\n\r\n")!=0 && *pointer!='\0');

    /* if the loop has been exited due to finding the character '\0', the request was not built correctly
    (there is no "\r\n\r\n") and ERR is returned */
    if (*pointer == '\0') {
        return ERR;
    }
...
int http_request_get_split(char *buf, size_t buflen, char **path, char **args, size_t *args_len) {
    ...
    /* sets length for both strings; when the previous loop has finished, "aux" points to the character '?',
    so path_len will be the address of "aux" minus the address of "buf"; then we increase the pointer "aux" to
    point to the beginning of the arguments string, whose length will be the total length of the initial string
    minus the length of the actual path plus 1 (due to the '?' character), which is now equal to "aux" - "buf"
    as "aux" now points to the character right after the '?' */
 ...
 * size_t *buflen: length of response, given as reference whose value is to be filled
 ...
 * void *body: body of the response given as a void pointer, to concatenate it at the end of response
...
int http_response_build(void **response, size_t *buflen, int version, int rescode, char *resp, size_t resp_len, int num_headers, struct http_pairs *headers, void *body, size_t body_len) {
    ...
    /* head_len is the length of the first part of the response, from the beginning to the sequence "\r\n\r\n",
    so the total length of response will be head_len + body_len */
    ...
    *buflen = head_len + body_len;
    ...
    /* copies the head of the response to the first head_len bytes of the memory reserved for the response */
    memcpy(buf, (void *)buf_aux1, head_len);

    /* copies the body of the response to the last body_len bytes of the memory reserved for the response,
    only if body_len > 0, because if body_len == 0 then, by our standards, there is no body and, indeed, body = NULL */
    if ((int)body_len > 0) {
        memcpy(buf+head_len, body, body_len);
    }

    /* assigns the memory address with the response information to the void pointer given by reference */
    *response = buf;
